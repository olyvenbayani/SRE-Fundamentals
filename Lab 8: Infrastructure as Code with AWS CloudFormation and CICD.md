...
### Step 16: Experiment with IaC Benefits

**Why?** Experience IaC advantages hands-on.

**Experiment 1: Create a Second Environment**

1. **Using CloudFormation Console:**
   - Create stack with same template
   - Stack name: `Lab8-IaC-Pipeline-Stack-Staging`
   - Change parameters:
     - GitHubBranch: `develop` (create this branch in GitHub first)
   - Creates a complete staging environment in minutes!

**Experiment 2: Update Infrastructure**

1. **Edit `pipeline-infrastructure.yaml`:**
   - Find the `BuildProject` resource
...
2. **Update the stack:**
   - CloudFormation > Stacks > Lab8-IaC-Pipeline-Stack > Update
   - Replace current template > Upload `pipeline-infrastructure.yaml`
   - Next > Next > Next > Update stack
   - CloudFormation updates ONLY the changed resource (CodeBuild)

**Explanation:** Infrastructure changes are code changes—version controlled, reviewable, testable!

**Experiment 3: Disaster Recovery Simulation**

1. **Delete the IaC stack:**
   - CloudFormation > Delete stack `Lab8-IaC-Pipeline-Stack`
   - Everything deleted in minutes

2. **Restore from "disaster":**
   - Create stack again with same template and parameters
   - Full environment restored in minutes!

**Try this with manual setup:** Would take an hour+ and might miss configurations.
...
**Delete IaC Resources (Easy):**
1. CloudFormation > Stacks > Select `Lab8-IaC-Pipeline-Stack`
2. Delete stack
3. Wait for deletion to complete (2-3 minutes)
4. All resources automatically deleted except:
   - S3 buckets with contents (must empty first)
   - GitHub connection (can be reused)

**Empty S3 buckets before deletion:**
```bash
aws s3 rm s3://lab8-iac-webapp-<your-account-id> --recursive
aws s3 rm s3://lab8-pipeline-artifacts-<your-account-id> --recursive
```

Then delete the stack again, or manually delete buckets from S3 console.

**Delete Manual Resources (if kept):**
- Follow Step 9 instructions
- Takes longer, must delete in correct order

**Delete GitHub Connection (Optional):**
- CodePipeline > Settings > Connections > Delete connection

**Explanation:** IaC cleanup is faster and guarantees nothing is left behind (except retained resources).

---

## Step 18: Reflect and Document Learning

**Why?** Solidify SRE concepts related to IaC.

Answer these questions in `learning-reflection.md`:

1. **What surprised you most about the manual approach?**
   - How many clicks? Potential for errors?

2. **What was the biggest "aha moment" with IaC?**
   - Speed? Reproducibility? Version control?

3. **How does IaC improve SRE practices?**
   - Reliability: Consistent configs reduce incidents
   - Repeatability: Identical dev/staging/prod environments
   - Observability: Infrastructure changes are auditable
   - Response: Faster disaster recovery

4. **Real-world scenarios where IaC helps:**
   - Scaling to multiple regions
   - Creating test environments for debugging
   - Rolling back infrastructure changes
   - Onboarding new team members

5. **What challenges might IaC introduce?**
   - Learning curve for CloudFormation syntax
   - Template complexity for large infrastructures
   - State management (CloudFormation handles this)

---

## Bonus Challenges (Optional)

**Challenge 1: Add CloudWatch Alarms to Template**
- Modify template to create alarms for pipeline failures
- Use SNS for email notifications when builds fail

**Challenge 2: Multi-Region Deployment**
- Create the same stack in another region
- Use parameters to customize region-specific settings

**Challenge 3: Implement Blue/Green Deployment**
- Modify template to create two S3 buckets (blue and green)
- Use CloudFormation parameters to switch traffic between them

**Challenge 4: Terraform Alternative**
- Convert the CloudFormation template to Terraform HCL
- Compare CloudFormation vs. Terraform for this use case
