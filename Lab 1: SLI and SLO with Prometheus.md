...
1. Availability: Percentage of requests that return HTTP 200.
   ...
   - Prometheus Query: sum(rate(http_server_requests_total{status="200"}[5m])) / sum(rate(http_server_requests_total[5m])) * 100
...
You've completed Lab 1! Save this markdown as `lab1-guide.md` for reference. If issues arise, ask your instructor. Ready for Lab 2?
