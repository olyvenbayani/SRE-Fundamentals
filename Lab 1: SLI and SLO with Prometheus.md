...
Create a new directory:textmkdir sre-lab1
cd sre-lab1
Inside this directory, we'll create the files below. Use a text editor like VS Code to make them.
...
In your terminal (in the sre-lab1 directory):textdocker-compose up -d --build
--build builds the app image.
-d runs in the background.

Wait a few seconds, then check if it's running:textdocker ps
You should see sample-app and prometheus.
...
Check metrics: http://localhost:3000/metrics – Raw data for Prometheus.
Open Prometheus UI: http://localhost:9090
In the "Expression" box, query http_server_requests_total and click "Execute". You should see request counts (it might be low if you haven't made many requests yet—try refreshing the app endpoints a few times).
